...
## FEIJÃO  ((Caldo gostoso têm alguma informação incorreta))
...
## ARROZ - RAMPINELLI, SABOR SUL E TIO JOÃO
...
## CAFÉ - CABOCLO, MELLITA E ITAMARATY
...
## AÇÚCAR - ALTO ALEGRE, ESTRELA E DOCEÇÚCAR
...
## MARGARINA - QUALY, DELÍCIA E DORIANA
...
## CARNE - PATINHO, COXÃO DURO/POSTA E COXÃO MOLE
...
##### SOMA DOS ITENS DA CESTA BÁSICA
...
produtos_CBA_long <- produtos_CBA %>%
  pivot_longer(
    cols = starts_with("media"),  # Seleciona todas as colunas que começam com 'media'
    names_to = "Tipo_Media",  # Nome da nova coluna para os nomes
    values_to = "Valor_Media"  # Nome da nova coluna para os valores
  )

grafico <- ggplot(produtos_CBA_long, aes(x = Data, y = Valor_Media, color = Tipo_Media)) +
  geom_line() +
  labs(x = "Data", y = "Valor da Media", title = "Variação das Médias ao Longo do Tempo") +
  theme_minimal() +
...
produtos <- ggplotly(grafico, tooltip = c("x", "y", "color"), dynamicTicks = TRUE)
print(produtos)
...
## atualizando no chart studio
Sys.setenv("plotly_username"="lucaassbauer")
Sys.setenv("plotly_api_key"="0rMyg1RsWoF1ysEHwAfl")

## abrindo chart studio
api_create(ggCBA, filename = "ggCBA")


##### GRÁFICO TESTE - VARIAÇÃO PERCENTUAL

# ------------ pequeno teste sobre variação
Percentual <- Cesta.basica %>% 
  mutate(variacao_percentual = round((Custo / lag(Custo) - 1) * 100, 2))

# plotando gráfico teste sobre variação da CBA
grafico2 <- ggplot(Percentual, aes(x = Data, y = variacao_percentual)) +
  geom_col() +
  labs(x = "Data", y = "Custo da cesta de alimentos", title = "Custo da cesta de alimentos por mês e ano")

GGPercentual <- ggplotly(grafico2, dynamicTicks = T, n = 12)

print(GGPercentual)

api_create(GGPercentual, filename = "GGPercentual")
# ----------------------------------------------------------------------------------------------
...
sal.min$Data <- as.Date(paste0(sal.min$Data, ".01"), format = "%Y.%m.%d")

colnames(sal.min)[2] <- "Salário Mínimo"

sal.min <- subset(sal.min, select = -X)
sal.min <- sal.min[, -3]

sal.min_filtrado <- sal.min %>%
  filter(Data >= as.Date("2018-09-01") & Data <= as.Date("2024-05-01"))


# Juntar os data frames
dados_completos <- merge(sal.min_filtrado, Cesta.basica, by = "Data", all = TRUE)


# Excluir as colunas 'Data', 'ano' e 'mes'
dados_completos <- subset(dados_completos, select = -c(Data, ano, mes))

# Dividir a coluna 'Custo' pela coluna 'Salário Mínimo' e multiplicar por 220
horas.trab <- dados_completos
horas.trab$horas_trabalhadas <- (dados_completos$Custo / dados_completos$`Salário Mínimo`) 

horas.trab$horas_trabalhadas <- round(horas.trab$horas_trabalhadas, 2)

horas.trab$horas_trabalhadas <- percent(horas.trab$horas_trabalhadas)

# Excluir as colunas 'Custo' e 'Salário Mínimo'
horas.trab <- subset(horas.trab, select = -c( `Salário Mínimo`))

participacao <- ggplot(horas.trab, aes(x = mes_ano)) +
  geom_line(aes(y = Custo, color = "Custo")) +
  geom_line(aes(y = horas_trabalhadas, color = "Horas Trabalhadas")) +
  labs(x = "Mês e Ano", y = "Valor", color = "Variável") +
  scale_color_manual(values = c("Custo" = "blue", "Horas Trabalhadas" = "red"),
                     labels = c("Custo", "Horas Trabalhadas")) +
  theme_minimal()
